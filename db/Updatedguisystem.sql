...
--
-- Host: 127.0.0.1
-- Generation Time: Feb 17, 2020 at 11:49 PM
...
CREATE ALGORITHM=UNDEFINED DEFINER=`` SQL SECURITY DEFINER VIEW `customer_view`  AS  select `c`.`custname` AS `custName`,`s`.`custNo` AS `custNo`,`s`.`transNo` AS `transNo` from (`sales` `s` left join `customer` `c` on(`c`.`custNo` = `s`.`custNo`)) ;
